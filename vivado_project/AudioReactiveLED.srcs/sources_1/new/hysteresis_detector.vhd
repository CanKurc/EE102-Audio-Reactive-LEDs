...
-- ============================================================================
-- Hysteresis Beat Detector
--
-- Uses a 2-state FSM with dual thresholds to produce a clean single-cycle
-- beat_detected pulse whenever the filtered audio crosses above THRESH_HIGH.
-- The detector must see the signal drop below THRESH_LOW before it can
-- fire again, preventing rapid re-triggering from noise.
--
-- States:
--   ARMED : waiting for a beat. Fires pulse if audio > THRESH_HIGH.
--   FIRED : beat just happened. Returns to ARMED when audio < THRESH_LOW.
-- ============================================================================
    ...
    begin
        if rising_edge(clk) then
            if reset = '1' then
                ...
                -- This is what makes beat_pulse a single-cycle pulse instead
                -- of a latching signal.
                beat_pulse <= '0';
