...
architecture Behavioral of beat_color_auto is
    ...
    -- three states now: fade, wait at dim, then breathe
    ...
    -- 1088 frames at 435 fps is about 2.5 seconds
    constant IDLE_LIMIT : integer := 1088;

    -- how many frames between each dimming step
    -- 30 frames at 435 fps is like 70 ms, full fade takes about half a second
    constant FADE_RATE  : integer := 30;

    -- INCREASED: how many frames between each breath step 
    -- 10 frames per step * 256 steps = 2560 frames per cycle = ~5.8 seconds per full breath
    ...
    -- '0' means breathing is going up, '1' means going down
                ...
                -- only update on frame ticks, not every 100 MHz cycle
                    ...
                    case state is
                        ...
                        when BREATHING =>
                            ...
                            -- FIX APPLIED HERE: Map 0-127 to 7-0 brightness.
                            -- breath_count(6 downto 4) extracts the top 3 bits, effectively dividing by 16.
                            -- This guarantees a safe value between 0 and 7.
